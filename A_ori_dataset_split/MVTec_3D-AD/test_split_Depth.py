            ...
            if clsname=="bagel":
                ...
                for split in ["test"]:
                    ...
                    # for label_name in ["good", "combined","contamination","cut","hole"]: #tire
                        ...
                        label_dir1 = os.path.join(label_dir + "/xyz")
                        # print(label_name)
                        if not os.path.exists(label_dir):
                            continue

                        # 遍历图片文件
                        for img_path in glob(os.path.join(label_dir1, "*.tiff")):  # 匹配 .jpg 或 .JPG
                            # 生成相对路径
                            # print(img_path)
                            gt_path1 = img_path.replace("/xyz/", "/gt/")
                            xyz_path2 = os.path.splitext(gt_path1)[0] + ".png" 
                            # print(xyz_path)
                            ...
                            # 构造 JSON 对象
                                json_obj = {
                                    ...
                                    "maskname":  relative_path1.replace("\\", "/")
                                }
                                # 将 JSON 对象写入文件，每行一个
                                f.write(json.dumps(json_obj, ensure_ascii=False) + "\n")
...
output_json_path = "/home/admin1/2Tsdb/lkf/uniform-3dad/dataset_split_code/MVTec3DAD_Depth_test_json/bagel.json"
