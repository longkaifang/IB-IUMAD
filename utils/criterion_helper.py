...
class FeatureMSELoss(nn.Module):
    ...
    def forward(self, input):
        ...
        feature_rec_xyz = input["feature_rec_xyz"]
        feature_align_xyz = input["feature_align_xyz"]
        return self.criterion_mse(feature_rec, feature_align)
        #选择不同的loss组合进行梯度更新，如下：
        # return self.criterion_mse(feature_rec, feature_align)+self.criterion_mse(feature_rec_xyz, feature_align_xyz)
