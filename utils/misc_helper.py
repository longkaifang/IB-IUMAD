...
def update_config(config):
    # update feature size
    _, reconstruction_type = config.net[2].type.rsplit(".", 1) # 输出UniAD
    if reconstruction_type == "UniAD":
        input_size = config.dataset.input_size #[224,224] 输入图像的高度和宽度
        outstride = config.net[1].kwargs.outstrides[0] # 步幅 16 输出步幅，表示特征图的下采样倍数
        assert (
        ...
        feature_size = [s // outstride for s in input_size] # [14,14]
        config.net[2].kwargs.feature_size = feature_size

    # update planes & strides
    backbone_path, backbone_type = config.net[0].type.rsplit(".", 1) # models.backbones  backbone_type:efficientnet_b4
